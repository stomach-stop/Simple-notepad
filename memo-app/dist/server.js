...
app.post("/memos", (req, res) => {
    ...
    db.run("INSERT INTO memos (title, content) VALUES (?, ?)", [title, content], function (err) {
        if (err)
            return res.status(500).json({ error: "メモの追加に失敗しました" });
...
app.put("/memos/:id", (req, res) => {
    ...
    db.run(`UPDATE memos SET title = COALESCE(?, title), content = COALESCE(?, content) WHERE id = ?`, [title, content, id], function (err) {
        if (err)
